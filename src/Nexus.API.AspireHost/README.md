...
1. Set `Nexus.API.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../Nexus.API.Infrastructure/Nexus.API.Infrastructure.csproj -s Nexus.API.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../Nexus.API.Infrastructure/Nexus.API.Infrastructure.csproj -s Nexus.API.Web.csproj
```
